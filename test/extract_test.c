#include "extract_test.h"

void test_extract() {
    racs_db* db = racs_db_instance();
    racs_db_open(db, "config.yaml");
    racs_result res = racs_db_exec(db, "EXTRACT 'test' 2025-02-09T22:51:52.213Z 2025-02-09T22:51:52.215Z");

    msgpack_unpacked msg;
    msgpack_unpacked_init(&msg);
    msgpack_unpack_next(&msg, (char*)res.data, res.size, 0);

    msgpack_object obj = msg.data;

    msgpack_str_assert("i16v", &obj.via.array.ptr[0].via.str);
    assert(obj.via.array.ptr[1].via.bin.size == 44100*4);

    msgpack_unpacked_destroy(&msg);
    racs_result_destroy(&res);
    racs_db_close(db);
}

void test_extract_no_data() {
    racs_db* db = racs_db_instance();
    racs_db_open(db, "config.yaml");
    racs_result res = racs_db_exec(db, "EXTRACT 'test' 2025-02-09T22:51:52.213Z 2025-02-09T22:51:52.212Z");

    msgpack_unpacked msg;
    msgpack_unpacked_init(&msg);
    msgpack_unpack_next(&msg, (char*)res.data, res.size, 0);

    msgpack_object obj = msg.data;

    msgpack_str_assert("i16v", &obj.via.array.ptr[0].via.str);
    assert(obj.via.array.ptr[1].via.bin.size == 0);

    msgpack_unpacked_destroy(&msg);
    racs_result_destroy(&res);
    racs_db_close(db);
}

void test_extract_error() {
    racs_db* db = racs_db_instance();
    racs_db_open(db, "config.yaml");
    racs_result res = racs_db_exec(db, "EXTRACT 'test' 2025-02-09T22:5 2025-02-09T22:51:52.212Z");

    msgpack_unpacked msg;
    msgpack_unpacked_init(&msg);
    msgpack_unpack_next(&msg, (char*)res.data, res.size, 0);

    msgpack_object obj = msg.data;

    msgpack_str_assert("error", &obj.via.array.ptr[0].via.str);

    msgpack_unpacked_destroy(&msg);
    racs_result_destroy(&res);
    racs_db_close(db);
}

void test_extract_invalid_num_args() {
    racs_db* db = racs_db_instance();
    racs_db_open(db, "config.yaml");
    racs_result res = racs_db_exec(db, "EXTRACT 'tests' 2025-02-09T22:5");

    msgpack_unpacked msg;
    msgpack_unpacked_init(&msg);
    msgpack_unpack_next(&msg, (char*)res.data, res.size, 0);

    msgpack_object obj = msg.data;

    msgpack_str_assert("error", &obj.via.array.ptr[0].via.str);

    msgpack_unpacked_destroy(&msg);
    racs_result_destroy(&res);
    racs_db_close(db);
}

void test_extract_invalid_arg_type() {
    racs_db* db = racs_db_instance();
    racs_db_open(db, "config.yaml");
    racs_result res = racs_db_exec(db, "EXTRACT 'tests' 3.2 1739141512213");

    msgpack_unpacked msg;
    msgpack_unpacked_init(&msg);
    msgpack_unpack_next(&msg, (char*)res.data, res.size, 0);

    msgpack_object obj = msg.data;

    msgpack_str_assert("error", &obj.via.array.ptr[0].via.str);

    msgpack_unpacked_destroy(&msg);
    racs_result_destroy(&res);
    racs_db_close(db);
}