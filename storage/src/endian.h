...
AUXTS_FORCE_INLINE uint64_t swap64_if_big_endian(uint64_t d) {
    return AUXTS_IS_LITTLE_ENDIAN ? d : __builtin_bswap64(d);
}

AUXTS_FORCE_INLINE uint32_t swap32_if_big_endian(uint32_t d) {
    return AUXTS_IS_LITTLE_ENDIAN ? d : __builtin_bswap32(d);
}

AUXTS_FORCE_INLINE uint16_t swap16_if_big_endian(uint16_t d) {
