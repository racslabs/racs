// RACS - Remote Audio Caching Server
// Copyright (c) 2025 RACS Labs, LLC. All rights reserved.
//
// Licensed under the RACS Source Available License (RACS-SAL-1.0).
// Non-commercial use only. Commercial use requires a paid license.
// Contact: sales@racslabs.com
//
// SPDX-License-Identifier: RACS-SAL-1.0

#include "parser.h"

void racs_parser_init(racs_parser *parser, const char *source) {
    parser->ptr = source;
    parser->curr = 0;
}

racs_token racs_parser_next_token(racs_parser *parser) {
    regmatch_t match;

    while (*parser->ptr != '\0' && isspace(*parser->ptr)) {
        racs_parser_advance(parser, 1);
    }

    if (parser->ptr == NULL || *parser->ptr == '\0') {
        return racs_parser_lex_token_eof();
    }

    if (*parser->ptr == '+' || *parser->ptr == '-') {
        if (racs_match_token(parser->ptr, RACS_REGEX_INT, &match)) {
            return racs_parser_lex_token_int64(parser, &match);
        }

        if (racs_match_token(parser->ptr, RACS_REGEX_FLOAT, &match)) {
            return racs_parser_lex_token_float64(parser, &match);
        }

    }

    if (isdigit(*parser->ptr)) {
        if (racs_match_token(parser->ptr, RACS_REGEX_TIME, &match)) {
            return racs_parser_lex_token_time(parser, &match);
        }

        if (racs_match_token(parser->ptr, RACS_REGEX_FLOAT, &match)) {
            return racs_parser_lex_token_float64(parser, &match);
        }

        if (racs_match_token(parser->ptr, RACS_REGEX_INT, &match)) {
            return racs_parser_lex_token_int64(parser, &match);
        }
    }

    if (*parser->ptr == '.') {
        if (racs_match_token(parser->ptr, RACS_REGEX_FLOAT, &match)) {
            return racs_parser_lex_token_float64(parser, &match);
        }
    }

    if (*parser->ptr == '\'') {
        if (racs_match_token(parser->ptr, RACS_REGEX_STR, &match)) {
            return racs_parser_lex_token_str(parser, &match);
        }
    }

    if (*parser->ptr == '|') {
        if (racs_match_token(parser->ptr, RACS_REGEX_PIPE, &match)) {
            return racs_parser_lex_token_pipe(parser, &match);
        }
    }

    if (isalpha(*parser->ptr) || *parser->ptr == '_') {
        if (racs_match_token(parser->ptr, RACS_REGEX_ID, &match)) {
            return racs_parser_lex_token_id(parser, &match);
        }
    }

    return racs_parser_token_error(parser);
}

int racs_match_token(const char *ptr, const char *pattern, regmatch_t *match) {
    regex_t regex;

    regcomp(&regex, pattern, REG_EXTENDED);
    int ret = regexec(&regex, ptr, 1, match, 0);

    regfree(&regex);

    return !ret;
}


void racs_parser_advance(racs_parser *parser, regoff_t step) {
    parser->ptr += step;
    parser->curr += step;
}

racs_token racs_parser_lex_token_time(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so;

    char *time_str = malloc(size + 1);
    snprintf(time_str, size + 1, "%s", parser->ptr);

    racs_token token;
    token.type = RACS_TOKEN_TYPE_TIME;
    token.as.time = racs_time_from_rfc3339(time_str);

    racs_parser_advance(parser, size);
    free(time_str);

    return token;
}

racs_token racs_parser_lex_token_str(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so - 2;

    racs_parser_advance(parser, 1);

    racs_token token;
    token.type = RACS_TOKEN_TYPE_STR;
    token.as.str.ptr = parser->ptr;
    token.as.str.size = size;

    racs_parser_advance(parser, size + 1);

    return token;
}

racs_token racs_parser_lex_token_id(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so;

    racs_token token;
    token.type = RACS_TOKEN_TYPE_ID;
    token.as.id.ptr = parser->ptr;
    token.as.id.size = size;

    racs_parser_advance(parser, size);

    return token;
}

racs_token racs_parser_lex_token_int64(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so;

    char *int_str = malloc(size + 1);
    snprintf(int_str, size + 1, "%s", parser->ptr);

    racs_token token;
    token.type = RACS_TOKEN_TYPE_INT;
    token.as.i64 = strtoull(int_str, NULL, 10);

    racs_parser_advance(parser, size);
    free(int_str);

    return token;
}

racs_token racs_parser_lex_token_float64(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so;

    char *float_str = malloc(size + 1);
    snprintf(float_str, size + 1, "%s", parser->ptr);

    racs_token token;
    token.type = RACS_TOKEN_TYPE_FLOAT;
    token.as.f64 = atof(float_str);

    racs_parser_advance(parser, size);
    free(float_str);

    return token;
}

racs_token racs_parser_lex_token_pipe(racs_parser *parser, regmatch_t *match) {
    regoff_t size = match->rm_eo - match->rm_so;

    racs_token token;
    token.type = RACS_TOKEN_TYPE_PIPE;

    racs_parser_advance(parser, size);

    return token;
}

racs_token racs_parser_lex_token_eof() {
    racs_token token;
    token.type = RACS_TOKEN_TYPE_EOF;
    return token;
}

racs_token racs_parser_token_error(racs_parser *parser) {
    sprintf(parser->error, "Invalid token at %d", parser->curr);
    racs_log_error(parser->error);

    racs_token token;
    token.type = RACS_TOKEN_TYPE_ERROR;
    token.as.err.msg = parser->error;

    return token;
}
