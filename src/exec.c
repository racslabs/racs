...
void racs_exec_init(racs_exec *exec) {
    exec->kv = racs_kvstore_create(19, racs_exec_hash, racs_exec_cmp, exec_destroy);
    ...
    racs_kvstore_put(exec->kv, strdup("EXPIRE"), racs_command_expire);
    racs_kvstore_put(exec->kv, strdup("TTL"), racs_command_ttl);
}
